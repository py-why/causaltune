...
from econml.score.ensemble_cate import EnsembleCateEstimator

# code adapted from https://github.com/microsoft/EconML/blob/main/econml/score/rscorer.py

class RScoreWrapper:
    
    def __init__(self, model_y, model_t, train_df, test_df, outcome, treatment, common_causes, effect_modifiers):
        ...
        self.train.fit(train_df[outcome].to_numpy(), train_df[treatment].to_numpy(), train_df[common_causes].to_numpy(),
                               train_df[effect_modifiers].to_numpy())
        self.test.fit(test_df[outcome].to_numpy(), test_df[treatment].to_numpy(), test_df[common_causes].to_numpy(),
...
class RScorer:
    ...
    def score(self, cate_estimate):
        ...
        effects = cate_estimate.reshape((-1, Y_res.shape[1], T_res.shape[1]))
