...
using builtin EconML functionality for that where it is available and bootstrapping where it is not (see [example notebook](https://github.com/transferwise/causaltune/blob/main/notebooks/Standard%20errors.ipynb)).
...
provided by the user do reflect all sources of confounding between the treatment and the outcome. See [here](https://github.com/transferwise/causaltune/blob/main/notebooks/CausalityDataset%20setup.ipynb) for a detailed explanation of causal graphs that are supported by CausalTune.
...
impact in the next iteration. CausalTune also serves as a variance reduction method leveraging the availability of any additional features. [Example notebook](https://github.com/transferwise/causaltune/blob/main/notebooks/AB%20testing.ipynb)
...
As in this case the propensity to treat is known for each customer, we [allow to explicitly supply it](https://github.com/transferwise/causaltune/blob/main/notebooks/Propensity%20Model%20Selection.ipynb)
...
The ERUPT technique [sample notebook](https://github.com/transferwise/causaltune/blob/main/notebooks/ERUPT%20under%20simulated%20random%20assignment.ipynb) re-weights the actual outcomes to produce an unbiased estimate of the average outcome that a fully random assignment would have yielded, with no actual additional group needed.
...
the assignment probability from the data). [Example notebook](https://github.com/transferwise/causaltune/blob/main/notebooks/Propensity%20Model%20Selection.ipynb)
...
and search over IV models in EconML to estimate the impact of the customer choosing to use it. To score IV model fits out of sample, we again use the [energy score](https://arxiv.org/abs/2212.10076). [Example notebook](https://github.com/transferwise/causaltune/blob/main/notebooks/Comparing%20IV%20Estimators.ipynb)
