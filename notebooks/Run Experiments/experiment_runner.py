...
from causaltune.datasets import generate_synthetic_data, generate_linear_synthetic_data, load_dataset
...
def parse_arguments():
    ...
    parser.add_argument('--metrics', nargs='+', default=['psw_frobenius_norm'], 
                        help='Metrics to use for evaluation')
    parser.add_argument('--datasets', nargs='+', default=['Small Linear_RCT'],
    ...
    parser.add_argument('--time_budget', type=int, default=None, 
                        help='Time budget for optimization')
    parser.add_argument('--components_time_budget', type=int, default=None, 
                        help='Time budget for component optimization')
    parser.add_argument('--identifier', default='', 
                        help='Additional identifier for output directory')
...
def run_experiment(args):
    ...
    for dataset in args.datasets:
        parts = dataset.split()
        if len(parts) < 2:
            raise ValueError(f"Invalid dataset format: {dataset}. Expected format: Size Name (e.g., Small Linear_RCT)")
        size = parts[0]
        name = ' '.join(parts[1:])
        ...
        for i_run in range(1, args.n_runs + 1):
            ...
            for metric in args.metrics:
                if 'KC' in dataset_name and 'KCKP' not in dataset_name:
                    ...
                    propensity_model = passthrough_model(cd.propensity_modifiers, include_control=False)
                else:
                ...
                with open(f"{out_dir}/{metric}_run_{i_run}_{dataset_name.replace(' ', '_')}.pkl", "wb") as f:
...
def compute_scores(ct, metric, test_df):
    ...
    for k in estimator_scores.keys():
        estimator_scores[k] = sorted(
            ...
            reverse=metric not in ["energy_distance", "psw_energy_distance", "codec", "frobenius_norm", "psw_frobenius_norm", "policy_risk"]
        )
...
def generate_plots(out_dir, metrics, datasets, n_runs):
    ...
    def plot_grid(title):
        fig, axs = plt.subplots(len(metrics), len(datasets), figsize=(20, 5*len(metrics)), dpi=300)
        ...
        plt.savefig(os.path.join(out_dir, f"CATE_grid.pdf"), format="pdf", bbox_inches="tight")
        plt.savefig(os.path.join(out_dir, f"CATE_grid.png"), format="png", bbox_inches="tight")
        plt.close()
    ...
    def plot_mse_grid(title):
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.savefig(os.path.join(out_dir, f"MSE_grid.pdf"), format="pdf", bbox_inches="tight")
        plt.savefig(os.path.join(out_dir, f"MSE_grid.png"), format="png", bbox_inches="tight")
        plt.close()
        ...
        ax_legend.axis('off')
        plt.savefig(os.path.join(out_dir, f"MSE_legend.pdf"), format="pdf", bbox_inches="tight")
        plt.savefig(os.path.join(out_dir, f"MSE_legend.png"), format="png", bbox_inches="tight")
        plt.close()
...
if __name__ == "__main__":
    ...
    generate_plots(run_experiment(args), args.metrics, args.datasets, args.n_runs)